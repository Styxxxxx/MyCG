...
def cut_image_patches(source_real, source_CG, target_dir, patch_dir, nb_input_images = 800,
    ...
    print "train: %d === test: %d === val: %d" % (nb_train,nb_test,nb_validation)

    for i in range(nb_train, nb_train + nb_validation):

        # img = cv2.imread(source_real + image_real[i] + ".TIF")

        img = cv2.imread(source_real + image_real[i] + ".jpg")
        # print "==============="
        # print source_real + image_real[i] + ".jpg"
        # print "read successful"
        print "==============="
        putting = target_dir + "/PG2000jpeg95/validation/Real/" + image_real[i] + ".jpg"
        ...
        #后面的 setpSize//10 需要根据实际图像大小调整 尽可能保持两个类的图像数量相等 后面的训练集与测试集也是一样
        ...
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf95/validation/CGG/', image_CG[i] + ".jpg", setpSize)
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf85/validation/CGG/', image_CG[i] + ".jpg", setpSize)
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf75/validation/CGG/', image_CG[i] + ".jpg", setpSize)
    ...
    for i in range(nb_train):
        ...
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf95/train/CGG/', image_CG[i] + ".jpg", setpSize)
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf85/train/CGG/', image_CG[i] + ".jpg", setpSize)
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf75/train/CGG/', image_CG[i] + ".jpg", setpSize)
    ...
    for i in range(nb_train + nb_validation, nb_train + nb_validation + nb_test):
        ...
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf95/test/CGG/', image_CG[i]+".jpg", setpSize)
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf85/test/CGG/', image_CG[i]+".jpg", setpSize)
        cut_one_image(source_CG, patch_dir + '/patchPG2000qf75/test/CGG/', image_CG[i]+".jpg", setpSize)
...
if __name__ == '__main__':
    ...
    # our data
    # Change to the source of real images
    source_real_tif_directory = "/home/qiaotong/data/columbia-prcg-datasets/google_images/"
    # Change to the source of CG images
    source_CG_directory = "/home/qiaotong/data/columbia-prcg-datasets/prcg_images/"
    # The directory where the database will be saved
    target_dir_test = '/home/qiaotong/sensors/dataset/'
    patch_dir_test = '/home/qiaotong/sensors/imagepatches/'
